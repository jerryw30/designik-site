...
export function AdminShell({ user, title, children, wide = false }: { user: { name: string; role: string }; title: string; children: React.ReactNode; wide?: boolean }) {
  return <main className="min-h-screen bg-[#f5f6f8] text-[#202126]">
    <aside className="fixed inset-y-0 left-0 z-40 w-64 overflow-y-auto bg-[#17181d] text-white"><div className="flex h-16 items-center border-b border-white/10 px-5 font-bold">DESIGNIK <span className="ml-1 text-pink-400">CMS</span></div><nav className="space-y-1 p-3">{groups.map(([label,href,icon]) => <Link key={href} href={href} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-white/65 transition hover:bg-white/10 hover:text-white"><span className="w-5 text-center text-white/45">{icon}</span>{label}</Link>)}</nav></aside>
