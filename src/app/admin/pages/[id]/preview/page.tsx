...
export default async function PagePreview({ params }: { params: Promise<{ id: string }> }) {
  ...
  const list = await db.select().from(sections).where(eq(sections.pageId,id)).orderBy(asc(sections.position));
  const hero = list.find((item) => item.type === "hero");
  return <SiteHome hero={heroContent(hero?.draftContent)} sections={list.map((s) => ({ id:s.id,type:s.type,visible:s.visible,content:s.draftContent }))} />;
}
