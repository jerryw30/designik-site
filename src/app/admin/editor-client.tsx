...
export default function EditorClient({ page, sections, initialHero }: { page: { id: string; title: string; slug: string; status: string }; sections: Section[]; initialHero: HeroContent }) {
  ...
  return <div className="grid min-h-[calc(100vh-4rem)] grid-cols-[260px_minmax(500px,1fr)_320px]">
    ...
    <section className="bg-[#25262c] p-5"><div className="mb-4 flex items-center justify-between"><div className="flex gap-2 rounded-lg bg-black/30 p-1 text-xs"><button className="rounded bg-white/10 px-3 py-1.5">Desktop</button><button className="px-3 py-1.5">Tablet</button><button className="px-3 py-1.5">Mobile</button></div><span className="text-xs text-white/40">Draft preview</span></div><div className="mx-auto h-[calc(100vh-9rem)] overflow-hidden rounded-xl bg-white shadow-2xl"><iframe ref={frame} title="Draft website preview" src="/admin/preview" className="h-full w-full border-0" /></div></section>
