      ...
      <div className="relative z-10 mx-auto max-w-[1280px] px-6 pt-16 pb-40 md:pt-20">
        ...
        <div className="flex flex-col items-start justify-between gap-8 border-b border-white/10 pb-12 md:flex-row md:items-center">
          <div className="flex items-center gap-3">
            <Image src={data.logo || assets.logo} alt={data.brand} width={48} height={48} className="h-11 w-11 brightness-0 invert" />
            <span className="font-display text-3xl font-bold uppercase tracking-tight">{data.brand}</span>
          </div>
        ...
        <div className="mt-12 grid grid-cols-2 gap-8 md:grid-cols-5">
          ...
          <div className="hidden items-start justify-center md:flex">
            <div className="flex h-24 w-24 items-center justify-center rounded-full border border-white/20">
              <Image src={data.logo || assets.logo} alt="" width={36} height={36} className="h-9 w-9 brightness-0 invert" />
            </div>
          </div>

          {/* newsletter */}
          <div className="col-span-2 md:col-span-1">
            <h3 className="font-display text-[15px] font-semibold uppercase tracking-wide">{data.newsletterHeading}</h3>
            <NewsletterForm />
        ...
        <p className="mt-14 text-center text-[13px] font-light text-white/60">{data.copyright}</p>
      </div>

      {/* desert scene */}
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[260px] w-full opacity-70">
        <Image src={data.backgroundImage || assets.desert} alt="" fill className="object-cover object-bottom" sizes="100vw" />
